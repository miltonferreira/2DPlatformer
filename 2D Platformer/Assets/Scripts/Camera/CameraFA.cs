...
public class CameraFA : MonoBehaviour
{
...
#if UNITY_EDITOR
[CustomEditor(typeof(CameraFA))]
public class CameraFollowEditor : Editor {
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();
        var script = (CameraFA) target;     // indica qual script vai pegar os valores
        ...
        if(script.setupComplete){
            ...
            if(script.ss == CameraFA.SetupState.None){
                if(GUILayout.Button("Start Settings Camera Values")){

                    script.ss = CameraFA.SetupState.Step1;
                }
            } else if(script.ss == CameraFA.SetupState.Step1){
                ...
                if(GUILayout.Button("Set Minimum Values")){
                    ...
                    script.ss = CameraFA.SetupState.Step2;  // indica proximo passo
                }

            } else if(script.ss == CameraFA.SetupState.Step2){
                ...
                if(GUILayout.Button("Set Maximum Values")){
                    ...
                    script.ss = CameraFA.SetupState.None;   // indica proximo passo
